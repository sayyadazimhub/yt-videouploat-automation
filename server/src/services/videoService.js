...
export const runVideoPipeline = async (projectId, storyData) => {
    ...
    try {
        ...
        // ── STEP 8: Auto-upload to YouTube (if enabled)
        if (project && project.auto_upload_youtube) {
            console.log(`🔄 Auto-upload to YouTube enabled for ${projectId}. Starting process...`);
            ...
            if (!account) {
                console.error(`❌ Auto-upload failed: YouTube account not connected.`);
                await AiVideoProject.updateOne({ _id: projectId }, {
                    youtube_status: "FAILED",
                    error_message: "Auto-upload failed: YouTube account not connected."
                });
            } else {
                await AiVideoProject.updateOne({ _id: projectId }, {
                ...
                try {
                    // Use the originally generated story metadata
                    console.log(`🧠 Reusing story metadata for ${projectId}...`);
                    const storyJson = JSON.parse(project.story_json);
                    
                    let ytTitle = storyJson.title || "AI Cinematic Video";
                    if (ytTitle.length > 95) ytTitle = ytTitle.substring(0, 95);
                    
                    const ytDescription = storyJson.description || "Generated by AI Cinematic Studio.";
                    const ytTags = ["aivideo", "shorts", storyJson.style || "Cinematic"];
                    ...
                    // Trigger upload (in background, though we are already in background)
                    console.log(`📤 Uploading to YouTube for ${projectId}...`);
                    
                    uploadVideo(account.refresh_token, finalVideoPath, { 
                        title: ytTitle, 
                    ...
                    .then(async (result) => {
                        ...
                        console.log(`✅ Auto-upload to YouTube complete for ${projectId}!`);
                    })
                    .catch(async (error) => {
                    ...
                    .finally(() => {
                        // We clean up here instead of immediately
                        try {
                            deleteProjectFiles(projectId);
                            console.log(`🧹 Cleaned up local files for ${projectId} after YT upload`);
                        } catch (_) {}
                    });
                    
                    // Return early so we don't hit the immediate cleanup below
                    return;
                } catch (metaErr) {
                    console.error("❌ Metadata generation failed:", metaErr);
                    await AiVideoProject.updateOne({ _id: projectId }, {
                        youtube_status: "FAILED",
                        error_message: "Metadata generation failed: " + metaErr.message
                    });
                }
            }
        }

        // Clean up local files now that it's in Cloudinary (if no YT auto-upload)
        try {
            deleteProjectFiles(projectId);
