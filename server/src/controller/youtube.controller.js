...
import { getAuthUrl, getTokens, getChannelInfo, uploadVideo, getVideoStatus } from "../services/youtubeService.js";
import fs from "fs";
...
export const generateMetadataController = async (req, res) => {
    try {
        ...
        const storyJson = project.story_json ? JSON.parse(project.story_json) : {};
        let title = storyJson.title || "AI Cinematic Video";
        if (title.length > 95) title = title.substring(0, 95);
        
        const metadata = {
            title: title,
            description: storyJson.description || "Generated by AI Cinematic Studio.",
            tags: ["aivideo", "shorts", storyJson.style || "Cinematic"]
        };
